...
$res1 = mysqli_query($mysqli, "SELECT * FROM continents");
$row1 =mysqli_fetch_assoc($res1);


$res2 = mysqli_query($mysqli, "SELECT * FROM countries");
$row2 =mysqli_fetch_assoc($res2);


$res3 = mysqli_query($mysqli, "SELECT * FROM imagedetails");
...
<body>
	...
	function check($row){
		...
		if(($continent ==='0')||(strcmp($continent,$row['ContinentCode'])===0)){
			...
			if(($title =="")||(strcmp($title,$row['Title'])==true)){
				return true;
			}
			}
		}
	...
	while($row =$res3 ->fetch_assoc()){
		if(check($row)){
			echo '<li><a href ="detail.php?id='.$row['ImageID'].'" class = "img_responsive">';
